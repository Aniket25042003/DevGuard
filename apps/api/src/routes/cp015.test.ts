...
import { registerDiagnosticsRoutes, type PreflightStatus, type RunsSummaryPort } from './diagnostics.routes.js';
import { registerFindingsRemediationRoutes, type RemediationSubmitPort } from './findings.routes.js';
  ...
  return createTransportKernel({
    ...
    authenticate: async ({ sessionToken }) =>
      sessionToken !== undefined
        ? { status: 'authenticated', principal: { userId: 'u-1', issuer: 'github', providerSubject: 'octo', authMethod: 'session' } as never }
        : { status: 'anonymous' },
  });
}

const preflight: PreflightStatus = { database: true, redis: true, trueforge: false, sandbox: false, github: true };
...
describe('CP015 diagnostics + remediation', () => {
  it('reports dependency preflight', async () => {
    ...
    const res = await k.app.request('/api/v1/diagnostics/preflight', { headers: { cookie: 'devguard_session=s1' } });
  ...
  it('lists a repository runs summary', async () => {
    const k = kernel();
    registerDiagnosticsRoutes(k, { preflight, runs: makeRuns });
    const res = await k.app.request('/api/v1/repositories/11111111-2222-4333-8444-555555555555/runs?limit=20', {
  ...
  it('remediation requires idempotency and fails closed on unregistered command', async () => {
    const k = kernel();
    const submit: RemediationSubmitPort = async () => ({ ok: false, code: 'COMMAND_UNKNOWN', detail: 'nope' });
    ...
    const ok = await k.app.request('/api/v1/findings/f-1/remediation', {
      method: 'POST',
      headers: { cookie: 'devguard_session=s1', 'content-type': 'application/json', 'idempotency-key': 'ik' },
      body: '{}',
    });
