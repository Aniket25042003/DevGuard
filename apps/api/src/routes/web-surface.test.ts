...
describe('CP018 web-surface aliases', () => {
  ...
  it('returns a GitHub App install URL when the app slug is configured', async () => {
    const k = kernel();
    registerWebSurfaceRoutes(
      k,
      { pool: undefined, config: { github: { appSlug: 'devguard' } } } as unknown as ApiContainer,
      approvals,
    );
    ...
    expect(body.installUrl).toBe('https://github.com/apps/devguard/installations/new');
  });

  it('fails closed on install intent without app slug', async () => {
    const k = kernel();
    register(k);
    const res = await k.app.request('/api/v1/github/installations/intents', {
      method: 'POST',
