...
| IDEMPOTENCY_KEY_REUSED        | concurrency   | 409  | no_retry             | This idempotency key was already used with a different request.                    | —                                 |
| MIGRATION_CHECKSUM_MISMATCH   | persistence   | 409  | no_retry             | An applied database migration no longer matches its recorded checksum.             | {version, name}                   |
| MIGRATION_DIRTY               | persistence   | 500  | no_retry             | The database schema is in a failed migration state; operator recovery is required. | {version}                         |
| SCHEMA_INCOMPATIBLE           | persistence   | 500  | no_retry             | The deployed database schema is not compatible with this service version.          | {reason}                          |
