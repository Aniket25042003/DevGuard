...
if (!manifestPath)
  ...
  try {
    ...
    const valid =
      ...
      /^[0-9a-f]{64}$/.test(manifest.migrationSetHash) &&
      Array.isArray(manifest.requiredChecks) &&
      manifest.requiredChecks.length > 0 &&
      new Set(['passed', 'failed', 'blocked', 'not_run']).has(manifest.providerContractStatus);
    checks.push({
      name: 'release.manifest',
      ok: valid,
      detail: valid ? 'validated' : 'invalid manifest fields',
    });
  } catch {
